...
class FeatureAgent(MahjongGBAgent):
    ...
    OFFSET_OBS = {
        ...
        'PREVALENT_WIND' : 1,
        'HAND' : 2,
        'EACH_CHI' : 6,#player0chi*4+player1chi*4+player2chi*4+player3chi*4
        'EACH_PENG' : 22,
        'EACH_GANG' : 26,
        'AN_GANG' : 30,
        'EACH_DISCARD_HISTORY' : 31,#28+28+28+28
        'REMAINING' : 143
    }
    ...
    def __init__(self, seatWind):
        ...
        self.packs = [[] for i in range(4)]#门前
        self.history = [[] for i in range(4)]
        self.tileWall = [21] * 4
        self.shownTiles = defaultdict(int)#已经亮出的牌
        self.wallLast = False
    ...
    def request2obs(self, request):
        ...
        if t[0] == 'Draw':
            ...
            self.tileWall[0] -= 1       #牌墙-1
            self.wallLast = self.tileWall[1] == 0   #如果是最后一张
            tile = t[1]                     #抓到的牌
            self.valid = []                 #可行操作
            ...
            self._hand_embedding_update()#更新手牌
            self._REMAINING_update()
            for tile in set(self.hand):
                self.valid.append(self.OFFSET_ACT['Play'] + self.OFFSET_TILE[tile])#可以打出
        ...
        if t[2] == 'Play':
            ...
            if p == 0:
                ...
                if not self.wallLast:
                    ...
                    if p == 3 and color in 'WTB':#判断能不能吃
    ...
    def _pack_update(self):
        self.obs[self.OFFSET_OBS['EACH_CHI'] :31] = 0
        
        for i in range(0,4):
            #d = defaultdict(int)
            cnt_CHI=0
            for s,tile,num in self.packs[i]:
                if s=='CHI':
                    self.obs[self.OFFSET_OBS['EACH_CHI'] +i*4+cnt_CHI, self.OFFSET_TILE[tile]-1:self.OFFSET_TILE[tile]+2] = 1
                    cnt_CHI+=1
                elif s=='Peng':
                    self.obs[self.OFFSET_OBS['EACH_PENG'] +i, self.OFFSET_TILE[tile]] = 1
                elif s=='Gang':
                    if i!=0 and tile!='CONCEALED':
                        self.obs[self.OFFSET_OBS['EACH_GANG'] +i, self.OFFSET_TILE[tile]] = 1
                    elif i==0:
                        if num!=0:
                            self.obs[self.OFFSET_OBS['EACH_GANG'] +i, self.OFFSET_TILE[tile]] = 1
                        else:
                            self.obs[self.OFFSET_OBS['AN_GANG'] , self.OFFSET_TILE[tile]] = 1

    def _history_update(self):
        self.obs[self.OFFSET_OBS['EACH_DISCARD_HISTORY'] :self.OFFSET_OBS['EACH_DISCARD_HISTORY'] + 4*28] = 0
        
        for i in range(0,4):
            #d = defaultdict(int)
            for j in range(len(self.history[i])):
                tile=self.history[i][j] 
                self.obs[self.OFFSET_OBS['EACH_DISCARD_HISTORY'] +i*28+j, self.OFFSET_TILE[tile]] = 1

    def _REMAINING_update(self):
        self.obs[self.OFFSET_OBS['REMAINING'] :self.OFFSET_OBS['REMAINING'] +4 ] = 0
        #d = defaultdict(int)
        for tile in self.shownTiles:
            self.obs[self.OFFSET_OBS['REMAINING'] : self.OFFSET_OBS['REMAINING'] + min(max(self.shownTiles[tile],0),4), self.OFFSET_TILE[tile]] = 1
    
    def _check_mahjong(self, winTile, isSelfDrawn = False, isAboutKong = False):#看是否能和
        try:
